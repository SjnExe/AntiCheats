...
*   **AutoMod System Review & Future Enhancements:** (Partially Completed; Further Enhancements Pending)
    ...
    *   **Next Steps:** Refine existing AutoMod rules and thresholds based on testing/feedback. Define AutoMod rules for checks that are currently disabled by default if they are enabled in the future. Consider further enhancements from the review document (`Dev/notes/AutoModReview_Findings.md`). Clarify `checkType` for `enableSelfHurtCheck` if AutoMod rules are desired for it.
