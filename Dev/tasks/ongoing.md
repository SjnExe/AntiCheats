...
## V. New Tasks Identified
- Implement `teleportSafe` actionType: The `teleportSafe` actionType defined in `automodConfig.js` for `movementNetherRoof` is not fully handled in `automodManager.js`'s main action processing logic. This needs implementation for the action to work as intended.
