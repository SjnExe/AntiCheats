...
commandManager.register({
    name: 'testui',
    description: 'For debugging UI actions directly.',
    category: 'Administration',
    permissionLevel: 0,
    execute: (player, args) => {
        if (args.length < 2) {
            player.sendMessage('§cUsage: !testui <actionName> <targetPlayerName>');
            return;
        }
        const actionName = args[0];
        const targetName = args[1];
        const actionFunction = uiActionFunctions[actionName];
        if (!actionFunction) {
            player.sendMessage(`§cUI Action '${actionName}' not found.`);
            return;
        }
        const targetPlayer = findPlayerByName(targetName);
        if (!targetPlayer) {
            player.sendMessage(`§cTarget player '${targetName}' not found.`);
            return;
        }
        player.sendMessage(`§eRunning test for action '${actionName}' on target '${targetName}'...`);
        try {
            actionFunction(player, { targetPlayer });
            player.sendMessage(`§aTest command executed. Check console for logs.`);
        } catch (e) {
            player.sendMessage(`§cTest command failed: ${e.message}`);
            console.error(`[TestUI] Error: ${e.stack}`);
        }
    }
});
