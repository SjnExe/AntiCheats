    ...
    for (const categoryName of categoryOrder) {
        ...
        if (commands && commands.length > 0) {
            ...
            for (const cmd of commands.sort((a, b) => a.name.localeCompare(b.name))) {
                helpMessage += `\n §b/exe:${cmd.name}§r: ${cmd.description}`;
            }
        }
    }
...
function showSpecificHelp(player, commandName) {
    ...
    let helpMessage = `§a--- Help: /exe:${cmd.name} ---\n`;
    helpMessage += `§eDescription§r: ${cmd.description}\n`;
    if (cmd.aliases && cmd.aliases.length > 0) {
        helpMessage += `§eAliases§r: ${cmd.aliases.map(a => `/exe:${a}`).join(', ')}\n`;
    }
...
customCommandManager.register({
    ...
    parameters: [
        { name: 'topic', type: 'string', description: 'The command to get help for.', optional: true }
    ],
    execute: (player, args) => {
        ...
        if (!topic) {
            ...
            showSpecificHelp(player, topic.toLowerCase());
        }
    }
});
