...
export async function execute(player, args, dependencies) {
    ...
    if (args.length < 1) {
        player.sendMessage(`§cUsage: ${prefix}invsee <playername>`); // Hardcoded string
        return;
    }
    ...
    if (!inventoryComponent?.container) { // Added optional chaining
        player.sendMessage(`§cCould not access ${foundPlayer.nameTag}'s inventory.`); // Hardcoded string
        return;
    }
    ...
    for (let i = 0; i < container.size; i++) {
        ...
        if (itemStack) {
            itemCount++;
            let nameTagText = itemStack.nameTag ? ` (Name: ${itemStack.nameTag})` : ''; // Hardcoded string
            let durabilityText = '';
            try {
                const durabilityComponent = itemStack.getComponent(ItemComponentTypes.Durability);
                if (durabilityComponent) {
                    durabilityText = ` (Dur: ${durabilityComponent.maxDurability - durabilityComponent.damage}/${durabilityComponent.maxDurability})`; // Hardcoded string
                }
            ...
            if (lore && lore.length > 0) {
                loreText = ` (Lore: ['${lore.join('\', \'')}'])`; // Hardcoded string
            }
            ...
            try {
                ...
                if (enchantableComponent) {
                    ...
                    if (enchantments.length > 0) {
                        ...
                        enchantsText = ` (Enchants: ${enchStrings.join(', ')})`; // Hardcoded string
                    }
                }
    ...
    try {
        ...
        invForm.title(`Inventory: ${foundPlayer.nameTag}`); // Hardcoded string
        invForm.body(inventoryDetails.trim());
        invForm.button1('Close'); // Hardcoded string
        ...
        player.sendMessage('§cAn error occurred while trying to display the inventory.'); // Hardcoded string
